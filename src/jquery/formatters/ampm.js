...
(function($) {
  ...
  $.timeAutocomplete.formatters.ampm.prototype = {
    ...
    hook_filterSource: function(el) {
      ...
      return (function(times, self) {
        return function(req, responseFn) {
          ...
          var do_has_am_pm_mind_read =
            (has_am || has_pm) && re.replace(/a|m|p/gi, "").length <= 2;
          ...
          if (self.options.from_selector) {
            trim_ampm_possibilities =
              self.detectAMPMFromInstanceOverlap() == self.options.am_text
                ? self.options.pm_text
                : self.options.am_text;
          }
          ...
          if (self_val) {
            a = $.grep(times, function(item) {
              var return_nil =
                ...
                (trim_ampm_possibilities &&
                  new RegExp(trim_ampm_possibilities, "gi").test(item)) ||
                // If we type in "1", don't show "11" and "12" possibilities
                (is_one && item.substring(0, 2) != "1:") ||
                (~self_val.toLowerCase().indexOf("p") &&
                  !~item.toLowerCase().indexOf("p")) ||
                (~self_val.toLowerCase().indexOf("a") &&
                  !~item.toLowerCase().indexOf("a"));
              if (return_nil) {
                return;
              }
    ...
    detectAMPMFromInstanceOverlap: function() {
      ...
      if (
        from_obj.postfix &&
        (~from_obj.postfix.toLowerCase().indexOf("a") ||
          ~from_obj.postfix.toLowerCase().indexOf("p"))
      ) {
        var from_ampm = ~from_obj.postfix.toLowerCase().indexOf("a")
          ? this.options.am_text
          : this.options.pm_text;
        ...
        if (from_hour == 12 && to_hour != 12) {
          trim_ampm_possibilities =
            from_ampm == this.options.am_text
              ? this.options.am_text
              : this.options.pm_text;
    ...
    generateTimes: function() {
      if (!this.options.times.length) {
        ...
        var date = new Date(
          ...
          1,
          1,
          this.options.start_hour - 1,
          start_minute - increment
        );
        var arr = [];
        var hour_len = 60;
        ...
        for (
          var i = 0, loop_int = hour_looper * (hour_len / increment);
          i <= loop_int;
          i++
        ) {
