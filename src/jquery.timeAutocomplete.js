...
(function($) {
  ...
  TimeAutocomplete.prototype = {
    ...
    _callAutocomplete: function() {
      this.options.auto_complete.source = this._callFormatterMethod(
        "filterSource",
        [this.el],
        function(req, responseFn) {
          throw new Error(
            "You must set a hook_filterSource method in your formatter."
          );
        }
      );
      ...
      if (typeof $.fn.autocomplete === "undefined") {
        throw new Error(
          "You need to include the jQuery UI bundle that has the Autocomplete plugin."
        );
      }
    ...
    _setupPlaceholder: function() {
      if (typeof this.el.attr("placeholder") === "undefined") {
        this.el.attr(
          "placeholder",
          this._callFormatterMethod("placeholderValue", [], "")
        );
      }
    },
    ...
    _keydownAutocomplete: function(e) {
      ...
      if (
        !~$.inArray(e.which, ignore_keydowns) &&
        (e.which == 8 ||
          (val.length > 1 &&
            !~val.indexOf("h") &&
            !~val.indexOf(":") &&
    ...
    _attacheUsableTimeData: function() {
      ...
      this.el.data(
        time_data_prop,
        this._callFormatterMethod("getUsableTimeValue", [val])
      );
    },
    ...
    setFormatter: function(formatter) {
      ...
      if (!$.timeAutocomplete.formatters[this.options.formatter]) {
        throw new Error(
          "Formatter: '" +
            formatter +
            "' was not found. Make sure you're loading it (formatters/" +
            this.options.formatter +
            ".js) after you load src/TimeAutocomplete.js"
        );
      } else {
        this._formatter = new $.timeAutocomplete.formatters[
          this.options.formatter
        ](this, this.options);
    ...
    _setValueAsTime: function() {
      ...
      if (val == "" && this.options.value) {
        ...
        val_parts.length == 3 &&
        this.isNumber(val_parts[0]) &&
        this.isNumber(val_parts[1]) &&
        this.isNumber(val_parts[2])
      ) {
        this.setTime(val);
      } else {
    ...
    setTime: function(time) {
      ...
      if (
        matched &&
        matched.length &&
        (matched[0].length == 5 || matched[0].length == 6)
      ) {
        ...
        throw new Error(
          "Setting a time must be in H:i:s format. Example: 03:30:00"
        );
      }
    },
    ...
    _getCurrentTimeAsValue: function() {
      ...
      var bound_times = formatter.options.times
        .slice()
        .concat(formatter.options.times);
      var entered_next_times = [];

      for (var i = 0, t = bound_times.length; i < t; i++) {
        var time = this._callFormatterMethod("getTime", [
          bound_times[i],
          fake_date_data
        ]);
        var next_time = bound_times[i + 1]
          ? this._callFormatterMethod("getTime", [
              bound_times[i + 1],
              fake_date_data
            ])
          : false;
        var already_entered_next_times = !!(
          $.inArray(next_time, entered_next_times) !== -1
        );
